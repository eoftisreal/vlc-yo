    ...
    , m_intf(ctx->getIntf())
{
    ...
    setIcon(iconVLC);
    setToolTip( qtr( "APOI media player" ));

    m_menu = std::make_unique<VLCMenu>( qtr( "APOI media player"), m_intf );
    ...
    else
    {
            ...
            ( m_notificationSetting == NOTIFICATION_MINIMIZED && (windowVisiblity == QWindow::Hidden || windowVisiblity == QWindow::Minimized)))
        {
            const auto showMessageTemplate = [this, &name](const auto &icon) {
                showMessage( qtr( "APOI media player" ), name, icon, 3000 );
            };
...
void VLCSystray::update()
{
    ...
    {
        m_menu->addAction(
            QIcon( ":/logo/vlc16.png" ), qtr( "&Hide APOI media player in taskbar" ),
            this, &VLCSystray::hideUpdateMenu);
    }
    else
    {
        m_menu->addAction(
            QIcon( ":/logo/vlc16.png" ), qtr( "Sho&w APOI media player" ),
